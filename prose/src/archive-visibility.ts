...
export class ArchiveVisibility {
  ...
  update(source: THREE.PerspectiveCamera, far: number, trackX: number, trackZ: number, extra: boolean): ArchiveCell[] {
    ...
    // 列的范围【必须】交给视锥决定，不要在这里另加窗口。
    //
    // 曾经把它掐到 3 列（理由是「单分类下多列会变镜厅」），结果顿挫很重：
    // 实测 16:9 屏幕上真正可见的是 lane 0..5 共 6 列，3 列只盖住中间
    // （ndc.x ≈ -0.30..0.27），左右两侧是空的；相机沿 trackX 一漂，
    // 列就在【屏幕中段】进出，而不是在边缘外进出。
    //
