...
function updateSelection(navigation?: ArchiveNavigation) {
  ...
  // 文苑是单列：这里的 lane 是「阵列落位」（data.ts 的 HOME_LANE = 2），
  // 不是分类下标，所以取列名必须先 wrap 回分类数组，否则 archiveColumns[2] 是 undefined。
  const laneIndex = wrap(lane, archiveColumns.length);
  ...
  columnTitle.update({ text: archiveColumns[laneIndex], animated: !prefs.reduced && mode === "archive" });
  $("#column-total").textContent = String(archiveColumns.length).padStart(2, "0");
  const singleColumn = archiveColumns.length < 2;
  $<HTMLButtonElement>('[data-action="column-prev"]').disabled = singleColumn;
  $<HTMLButtonElement>('[data-action="column-next"]').disabled = singleColumn;
...
function renderResults() {
  const results = records
    ...
    .filter(
      ({ r }) =>
        ...
        `${r.id} ${r.title} ${r.subtitle} ${r.description} ${r.tag} ${r.date}`
          .toLowerCase()
  ...
  $("#search-results").innerHTML = results.length
    ? results
        .map(
          ({ r, i }) =>
            `<button class="result-row" data-result="${i}"><span class="result-name"><b>${r.id}</b><span>${escapeHtml(r.title)}<small>${escapeHtml(r.subtitle || r.date)}</small></span></span><span>${escapeHtml(r.tag)}</span><span>${escapeHtml(r.date)} <i>↗</i></span></button>`,
        )
        .join("")
