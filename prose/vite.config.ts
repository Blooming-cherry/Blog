...
export default defineConfig(({ mode }) => ({
  // 文苑部署在 blog.adaydream.cn/prose/（博客的子目录），所以一律走相对路径
  // ——原版非 wallpaper 模式是 "/"，那是给站点根部署用的。
  // PWA 的 Service Worker 用 self.registration.scope 自行推导所有 URL
  // （见 src/asset-url.ts 的说明），不假定站点根，因此这里不需要额外改动。
  base: "./",
  define: {
